...
const nelReportSchema = z.object({
  age: z.number().min(0),
  type: z.enum(['network-error', 'dns.unreachable', 'dns.name_not_resolved', 'dns.failed', 'dns.address_changed', 'tcp.timed_out', 'tcp.closed', 'tcp.reset', 'tcp.refused', 'tcp.aborted', 'tcp.address_invalid', 'tcp.address_unreachable', 'tcp.failed', 'tls.version_or_cipher_mismatch', 'tls.bad_client_auth_cert', 'tls.cert.name_invalid', 'tls.cert.date_invalid', 'tls.cert.authority_invalid', 'tls.cert.invalid', 'tls.cert.revoked', 'tls.cert.pinned_key_not_in_cert_chain', 'tls.protocol.error', 'tls.failed', 'http.error', 'http.protocol.error', 'http.response.invalid', 'http.response.redirect_loop', 'http.failed', 'abandoned']),
  url: z.string().url(),
