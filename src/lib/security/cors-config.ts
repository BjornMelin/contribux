...
export const CorsConfigSchema = z.object({
  ...
  // Allowed headers
  allowedHeaders: z.union([z.literal('*'), z.array(z.string())]),
  ...
  handlePreflight(request: NextRequest): NextResponse {
    ...
    if (
      ...
      !this.config.methods.includes(
        requestMethod as 'GET' | 'POST' | 'PUT' | 'DELETE' | 'PATCH' | 'OPTIONS' | 'HEAD'
      )
    ) {
...
export class DynamicCorsConfig {
   ...
   */
  createDynamicMiddleware(routeConfig: (request: NextRequest) => string | CorsConfig | null) {
    return async (request: NextRequest): Promise<NextResponse | null> => {
 ...
 */
export async function logCorsViolation(request: NextRequest, reason: string): Promise<void> {
  const origin = request.headers.get('origin')
 ...
 */
export function checkSuspiciousCorsPatterns(request: NextRequest): {
  suspicious: boolean
  reasons: string[]
} {
  ...
  if (origin && referer) {
    try {
      ...
      // Ignore invalid URL formats - malformed origin/referer headers should not break security check
    }
  }
...
/**
 * CORS Security Monitor Functions
 * Provides security monitoring and validation for CORS operations
 */

// Export the existing functions with consistent naming for backward compatibility
export const checkSuspiciousPatterns = checkSuspiciousCorsPatterns
export const logViolation = logCorsViolation
