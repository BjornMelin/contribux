...
import { config, validateConfig, authConfig, webauthnConfig, oauthConfig, auditConfig, cryptoConfig, databaseConfig } from './index'
...
export function testConfigurationSystem(): boolean {
  try {
    ...
    if (allPassed) {
      ...
      console.log(`  - Rate limit: ${authConfig.rateLimit.max} requests per ${authConfig.rateLimit.windowMs}ms`)
