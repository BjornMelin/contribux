...
export function createAuthError(
  ...
  const errorMap = {
    ...
    invalid_token: {
      ...
      developmentDetails: 'JWT token signature verification failed or token structure is invalid.',
...
export function createDatabaseError(
  ...
  const config = errorMap[type]
  return createError(config.code, config.message, 'database', config.severity, {
    originalError,
    context,
    actionableSteps: config.actionableSteps,
    developmentDetails: config.developmentDetails,
...
export function createValidationError(
  ...
  return createError('VALIDATION_ERROR', 'Invalid input data provided.', 'validation', 'medium', {
    context: {
      ...context,
      fieldErrors,
 ...
 */
export function toHttpResponse(error: EnhancedError): NextResponse {
 ...
 */
export function logError(error: EnhancedError, request?: NextRequest): void {
  const _logData = {
    correlationId: error.correlationId,
    code: error.code,
    message: error.message,
  ...
  if (isProduction) {
    ...
    // This will be implemented when production monitoring is configured
    // Expected to capture error with correlation ID and context
  } else {
    // Development: error details already logged above with full context
    // Enhanced debugging information is safely available in dev environment
  }
}
 ...
 */
export function withEnhancedErrorHandling<T extends readonly unknown[]>(
  ...
  return async (...args: T): Promise<NextResponse> => {
    try {
      ...
      if (error && typeof error === 'object' && 'correlationId' in error) {
        ...
        logError(enhancedError, args[0] as NextRequest)
        return toHttpResponse(enhancedError)
      }

      // Handle Zod validation errors
      if (error instanceof ZodError) {
        const enhancedError = createValidationError(error)
        logError(enhancedError, args[0] as NextRequest)
        return toHttpResponse(enhancedError)
      }
...
/**
 * ErrorHandler object that groups all error handling functions
