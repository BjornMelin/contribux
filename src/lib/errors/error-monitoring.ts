...
import { ErrorClassification, ErrorCategory, ErrorSeverity } from './error-classification'
...
export class ErrorMonitor {
  ...
  async track(
    ...
    this.errorPatterns.set(
      patternKey,
      (this.errorPatterns.get(patternKey) || 0) + 1
    )
  ...
  getTrends(hours: number = 24): Array<{
    ...
    for (let i = 0; i < hours; i++) {
      ...
      const intervalErrors = this.errors.filter(
        e => e.timestamp.getTime() >= intervalStart && 
             e.timestamp.getTime() < intervalEnd
      )
