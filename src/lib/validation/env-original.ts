...
// Simplified entropy validation for JWT secrets
function isWeakSecret(secret: string): boolean {
  // Check for obviously weak patterns
  const lowerSecret = secret.toLowerCase()

  // Simple checks for common weak patterns
  const weakPatterns = [
    'test',
    'demo',
    'example',
  ...
  // Check for very simple repeated characters (like 'aaaaaaa...')
...
/**
 * Calculate Shannon entropy for encryption key strength validation
 */
...
/**
 * Check for repeated patterns in encryption keys
 */
function hasRepeatedPattern(key: string): boolean {
  // Check for simple repeating patterns
  if (key.length < 16) return false

  // Check for repeated substrings
  for (let i = 2; i <= key.length / 2; i++) {
    const pattern = key.substring(0, i)
    const repeated = pattern.repeat(Math.floor(key.length / i))

    if (key.startsWith(repeated) && repeated.length >= key.length * 0.7) {
      return true
    }
  }
