...
async function verifyHealthEndpoint() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.log(
        `     Overall Status: ${healthData.status} ${healthData.status === 'healthy' ? '✅' : '❌'}`
      )
      console.log(
        `     Database: ${healthData.services?.database} ${healthData.services?.database === 'connected' ? '✅' : '❌'}`
      )
      console.log(
        `     WebAuthn: ${healthData.services?.webauthn} ${healthData.services?.webauthn === 'available' ? '✅' : '❌'}`
      )
      console.log(
        `     Rate Limiting: ${healthData.services?.rateLimit} ${healthData.services?.rateLimit === 'active' ? '✅' : '❌'}`
      )
      console.log(
        `     Security Headers: ${healthData.services?.securityHeaders} ${healthData.services?.securityHeaders === 'enabled' ? '✅' : '❌'}`
      )
...
verifyHealthEndpoint().catch(error => {
