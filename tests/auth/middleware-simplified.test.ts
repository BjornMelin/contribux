...
import { authMiddleware } from '@/lib/auth/middleware-simplified'
...
describe('Simplified Auth Middleware', () => {
  ...
  describe('Rate Limiting', () => {
    it('should apply rate limiting to all requests', async () => {
      ...
      for (let i = 0; i < 100; i++) {
        Object.defineProperty(requests[i], 'ip', { value: '127.0.0.1' })
        const response = await authMiddleware(requests[i])
        expect(response).toBeUndefined()
      }

      // 101st request should be rate limited
      Object.defineProperty(requests[100], 'ip', { value: '127.0.0.1' })
      const response = await authMiddleware(requests[100])
      expect(response).toBeInstanceOf(NextResponse)
