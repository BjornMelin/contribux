...
import { ErrorMonitor, AlertingSystem, ErrorDashboard } from '@/lib/errors/error-monitoring'
...
// Mock API handlers
const mockHealthHandler = async (req: NextRequest) => {
  try {
...
describe('Error Monitoring Integration Tests', () => {
  ...
  describe('Analytics API', () => {
    it('should filter analytics by category', async () => {
      ...
      const req = new Request('http://localhost:3000/api/monitoring/analytics?category=database_connection')
  ...
  describe('Real-time Monitoring', () => {
    it('should track error velocity', async () => {
      ...
      expect(report.recommendations).toContain(
        expect.stringContaining('Error trend is increasing')
      )
    })
  ...
  describe('Component Health Tracking', () => {
    it('should track individual component health', async () => {
      ...
      const dbErrors = Array(10).fill(null).map(() => ({
        category: ErrorCategory.DATABASE_CONNECTION,
        severity: ErrorSeverity.HIGH,
        isTransient: true,
        recoveryStrategies: [],
      ...
      const githubErrors = Array(5).fill(null).map(() => ({
        category: ErrorCategory.GITHUB_API_ERROR,
        severity: ErrorSeverity.MEDIUM,
        isTransient: true,
        recoveryStrategies: [],
  ...
  describe('Incident Management', () => {
    it('should generate incident reports', async () => {
      ...
      const incidentErrors = Array(50).fill(null).map((_, i) => ({
