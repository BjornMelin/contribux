...
import { GitHubAuthenticationError, GitHubRateLimitError } from '@/lib/github/errors'
import type { GitHubClientConfig } from '@/lib/github/interfaces'
...
describe.skipIf(skipRealAPITests)('Real GitHub API Integration Tests', () => {
  ...
  afterEach(async () => {
    ...
    if (metrics.apiCalls.length > 0) {
      const avgDuration = metrics.apiCalls.reduce((sum, call) => sum + call.duration, 0) / metrics.apiCalls.length
      const cachedCalls = metrics.apiCalls.filter(call => call.cached).length
      const cacheHitRate = metrics.apiCalls.length > 0 ? (cachedCalls / metrics.apiCalls.length) * 100 : 0
  ...
  describe('Authentication Flow Validation', () => {
    it('should authenticate with Personal Access Token and verify scopes', async () => {
      ...
      metrics.apiCalls.push({
        ...
        statusCode: 200,
        cached: false,
        rateLimitRemaining: parseInt(user.headers['x-ratelimit-remaining'] || '0')
      })
      ...
      console.log(`   API calls remaining: ${user.headers['x-ratelimit-remaining']}/${user.headers['x-ratelimit-limit']}`)
    })
    ...
    it('should handle token validation and error scenarios', async () => {
      ...
      try {
        ...
        await expect(
          invalidClient.rest.users.getAuthenticated()
        ).rejects.toThrow()
  ...
  describe('Repository Operations with Real Data', () => {
    it('should list repositories with pagination and caching', async () => {
      ...
      metrics.apiCalls.push({
        ...
        cached: false,
        rateLimitRemaining: parseInt(repos1.headers['x-ratelimit-remaining'] || '0')
      })
    ...
    it('should handle organization repositories and permissions', async () => {
      ...
      try {
        ...
        metrics.apiCalls.push({
          ...
          cached: false,
          rateLimitRemaining: parseInt(orgRepos.headers['x-ratelimit-remaining'] || '0')
        })
        ...
        if (orgRepos.data.length > 0) {
          ...
          console.log(`   Detailed repo info: ${repo.full_name} (${repo.language || 'No language'})`)
        }
      } catch (error: any) {
        metrics.errors.push({
          endpoint: `/orgs/${org}/repos`,
  ...
  describe('GraphQL API with Real Queries', () => {
    it('should execute GraphQL queries with point tracking', async () => {
      ...
      console.log(`   Rate limit: ${result.rateLimit.remaining}/${result.rateLimit.limit} remaining`)
  ...
  describe('Rate Limiting Behavior', () => {
    ...
    it('should handle secondary rate limits gracefully', async () => {
      ...
      for (let i = 0; i < requestCount; i++) {
        const promise = client.rest.users.getAuthenticated()
          .then(response => ({
            index: i,
            duration: Date.now(),
      ...
      console.log(`✅ Rapid requests completed: ${successfulRequests.length} success, ${failedRequests.length} failed`)
  ...
  describe('Error Handling and Recovery', () => {
    it('should handle network timeouts and retry logic', async () => {
      ...
      try {
        ...
        metrics.apiCalls.push({
          ...
          cached: false,
          rateLimitRemaining: parseInt(result.headers['x-ratelimit-remaining'] || '0')
        })
      } catch (error: any) {
    ...
    it('should handle API quota and usage scenarios', async () => {
      ...
      const requestsUsed = rateLimit.resources.core.remaining - rateLimitAfter.resources.core.remaining
      
      console.log(`✅ API quota tracking verified in ${duration}ms`)
      console.log(`   Requests used: ${requestsUsed}`)
      console.log(`   Remaining: ${rateLimitAfter.resources.core.remaining}/${rateLimitAfter.resources.core.limit}`)
  ...
  describe('Performance and Memory Validation', () => {
    it('should maintain performance under typical load', async () => {
      ...
      console.log(`   Operations per second: ${((operationCount / totalDuration) * 1000).toFixed(2)}`)
...
export function validateIntegrationTestEnvironment(): {
  ...
  if (!process.env.GITHUB_APP_PRIVATE_KEY) {
    warnings.push('GITHUB_APP_PRIVATE_KEY not set - GitHub App authentication tests will be skipped')
  }
