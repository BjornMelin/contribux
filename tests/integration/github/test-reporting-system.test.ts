...
import { MetricsCollector, MemoryProfiler } from '../infrastructure/metrics-collector'
import { createIntegrationTestReporter } from '../infrastructure/reporter'
import { createPerformanceAnalyzer } from '../infrastructure/performance-analyzer'
...
describe.skipIf(skipReportingTests)('Test Reporting and Metrics Collection System', () => {
  ...
  describe('Metrics Collection Accuracy', () => {
    it('should accurately collect API call metrics', async () => {
      ...
      expect(successfulOperations.length + failedOperations.length).toBe(REPORTING_TEST_CONFIG.TEST_OPERATIONS)
      ...
      const recordedTotalDuration = Object.values(metrics.apiCalls.byEndpoint).reduce(
        (sum, count) => sum + (count * metrics.apiCalls.averageDuration), 0
      )
      const actualTotalDuration = results.reduce((sum, r) => sum + r.duration, 0)
      
      // Allow some variance in timing measurements
      const timingVariance = Math.abs(recordedTotalDuration - actualTotalDuration) / actualTotalDuration
      expect(timingVariance).toBeLessThan(0.2) // Less than 20% variance

      console.log(`✅ API metrics collection validated: ${metrics.apiCalls.total} calls, ${metrics.apiCalls.averageDuration.toFixed(2)}ms avg`)
    })
    ...
    it('should accurately track cache performance', async () => {
      ...
      console.log(`✅ Cache metrics validated: ${metrics.cache.hits} hits, ${metrics.cache.misses} misses, ${(metrics.cache.hitRate * 100).toFixed(1)}% hit rate`)
    })
    ...
    it('should monitor memory usage patterns accurately', async () => {
      ...
      for (let i = 0; i < 15; i++) {
        memoryOperations.push(
          testClient.rest.repos.listForAuthenticatedUser({ per_page: 10 })
            .then(() => {
      ...
      const afterGCGrowth = memoryProfiler.measureAfterGC()
      ...
      expect(Math.abs(metrics.memory.growth)).toBeLessThan(REPORTING_TEST_CONFIG.MEMORY_THRESHOLD_MB * 1024 * 1024)

      console.log(`✅ Memory monitoring validated: peak=${(metrics.memory.peak / 1024 / 1024).toFixed(2)}MB, growth=${(metrics.memory.growth / 1024 / 1024).toFixed(2)}MB`)
    })
  })
  ...
  describe('Performance Analysis and Quality Gates', () => {
    it('should validate performance thresholds for CI/CD gates', async () => {
      ...
      const qualityGates = {
        averageResponseTime: metrics.apiCalls.averageDuration < REPORTING_TEST_CONFIG.PERFORMANCE_THRESHOLD_MS,
        ...
        memoryGrowth: Math.abs(metrics.memory.growth) < REPORTING_TEST_CONFIG.MEMORY_THRESHOLD_MB * 1024 * 1024
      }
      ...
      expect(metrics.apiCalls.averageDuration).toBeLessThan(REPORTING_TEST_CONFIG.PERFORMANCE_THRESHOLD_MS * 2) // Allow 2x threshold for test environment
      expect(metrics.apiCalls.errorRate).toBeLessThan(REPORTING_TEST_CONFIG.ERROR_RATE_THRESHOLD * 2)

      console.log(`✅ Quality gates assessment: ${passedGates}/${totalGates} passed (${(passRate * 100).toFixed(1)}%)`)
    })
    ...
    it('should generate performance trend analysis', async () => {
      ...
      console.log(`   Response time trend: ${responseTimeTrend > 0 ? '+' : ''}${responseTimeTrend.toFixed(2)}ms per interval`)
      console.log(`   Cache hit rate trend: ${cacheHitTrend > 0 ? '+' : ''}${(cacheHitTrend * 100).toFixed(2)}% per interval`)
...
function generatePerformanceRecommendations(responseTimeTrend: number, cacheHitTrend: number): string[] {
  const recommendations = []
  
  if (responseTimeTrend > 10) {
    recommendations.push('Response times are increasing. Consider optimizing API calls or increasing rate limit tokens.')
  }
  
  if (cacheHitTrend < -0.1) {
    recommendations.push('Cache hit rate is declining. Review cache TTL settings and cache key strategies.')
  }
...
function generateTimeSeriesData(metric: string, points: number): Array<{ timestamp: number; value: number }> {
  const data = []
  const now = Date.now()
