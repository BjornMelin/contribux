...
import { GitHubAuthenticationError, GitHubTokenExpiredError } from '../../../src/lib/github/errors'
import { describeIntegration, integrationTest, withRetry, measurePerformance, waitForCondition } from '../infrastructure/test-runner'
import type { IntegrationTestContext } from '../infrastructure/test-config'
import type { GitHubClientConfig, TokenInfo } from '../../../src/lib/github/interfaces'

describeIntegration('GitHub Authentication Flows', (getContext) => {
      ...
      try {
        ...
        if (context.metricsCollector) {
          ...
          context.metricsCollector.recordRateLimit(
            'core',
            parseInt(user.headers['x-ratelimit-remaining']),
            parseInt(user.headers['x-ratelimit-limit'])
          )
        }
      } finally {
  ...
  describe('GitHub App Authentication', () => {
    ...
    integrationTest('should generate valid JWT for GitHub App', async () => {
      if (skipGitHubApp) {
      ...
      const client = new GitHubClient({
        auth: {
          type: 'app',
          appId: parseInt(context.env.GITHUB_APP_ID!),
      ...
      try {
        ...
        expect(app.data.id).toBe(parseInt(context.env.GITHUB_APP_ID!))
    ...
    integrationTest('should handle installation token exchange', async () => {
      ...
      const client = new GitHubClient({
        auth: {
          ...
          appId: parseInt(context.env.GITHUB_APP_ID!),
          privateKey: context.env.GITHUB_APP_PRIVATE_KEY!,
          installationId: parseInt(context.env.GITHUB_APP_INSTALLATION_ID!)
        }
      })

      try {
        // Authenticate as installation
        await client.authenticateAsInstallation(parseInt(context.env.GITHUB_APP_INSTALLATION_ID!))

        // Verify installation token works
        const installation = await client.rest.apps.getInstallation({
          installation_id: parseInt(context.env.GITHUB_APP_INSTALLATION_ID!)
        })
        expect(installation.data).toBeDefined()
        expect(installation.data.id).toBe(parseInt(context.env.GITHUB_APP_INSTALLATION_ID!))
    ...
    integrationTest('should handle JWT expiration and refresh', async () => {
      if (skipGitHubApp) {
  ...
  describe('Error Recovery and Retry Logic', () => {
    ...
    integrationTest('should handle network timeouts during authentication', async () => {
