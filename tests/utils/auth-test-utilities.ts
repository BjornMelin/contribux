...
// OAuth Flow Simulation Functions
 ...
 */
export function simulateGitHubOAuth() {
 ...
 */
export function simulateGoogleOAuth() {
  const state = generateStateParameter()
  const code = generateAuthorizationCode()
 ...
 */
export function simulateOAuthError(
  errorType: 'access_denied' | 'invalid_request' | 'server_error'
) {
...
type SqlQueryFunction = (...args: unknown[]) => Promise<unknown[]>

export class DatabaseMockHelper {
  private mockSql: MockedFunction<SqlQueryFunction>

  constructor(mockSql: MockedFunction<SqlQueryFunction>) {
   ...
   */
  mockConcurrentOperations(results: (Error | unknown)[]): void {
 ...
 */
export function getMaliciousInputSamples() {
  return {
    xssAttempts: [
 ...
 */
export function simulateSessionHijacking(validSession: Session) {
  return {
 ...
 */
export function generateRateLimitingScenarios(userSession: Session) {
  return {
 ...
 */
export async function simulateConcurrentAuth(
  authOperation: () => Promise<unknown>,
  concurrentUsers: number
): Promise<{
  results: Array<{ success: boolean; result?: unknown; error?: unknown; userIndex: number }>
