/**
 * MSW-Based API Routes Test Suite
 * Tests API routes using MSW for HTTP interception with proper Next.js API route patterns
 */
...
// Mock API implementations that simulate our actual API routes
const BASE_URL = 'http://localhost:3000'

const server = setupServer(
  // Health check endpoint - mock the actual health check logic
  ...
  // Health check with database error simulation
  ...
  // Search opportunities endpoint with proper validation
  http.get(`${BASE_URL}/api/search/opportunities`, ({ request }) => {
    ...
    // Validate type enum
    const validTypes = ['bug_fix', 'feature', 'documentation', 'test', 'refactor', 'security']
  ...
  // Search opportunities with database error simulation
...
describe('API Routes - MSW HTTP Integration Tests', () => {
  beforeAll(() => {
  ...
  describe('Search Opportunities API', () => {
    ...
    it('should validate difficulty enum values', async () => {
      const response = await fetch(`${BASE_URL}/api/search/opportunities?difficulty=invalid`)
      expect(response.status).toBe(400)
    ...
    it('should validate type enum values', async () => {
      const response = await fetch(`${BASE_URL}/api/search/opportunities?type=invalid`)
      expect(response.status).toBe(400)
