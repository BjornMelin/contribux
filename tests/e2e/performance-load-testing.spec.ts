  ...
  test.describe('API Performance Testing', () => {
    test('should handle API requests efficiently', async ({ page }) => {
      ...
      for (const endpoint of apiEndpoints) {
        ...
        console.log(
          `${endpoint.path} response time: ${performance.responseTime}ms (status: ${performance.status})`
        )
    ...
    test('should handle concurrent API requests', async ({ page }) => {
      ...
      const promises = concurrentRequests.map(async (endpoint, _index) => {
    ...
    test('should maintain performance under sustained load', async ({ page }) => {
      ...
      console.log('Sustained load test completed. Metrics:', {
        ...
        averageOperationTime:
          performanceMetrics.reduce((sum, m) => sum + m.operationTime, 0) /
      ...
      const secondHalfAvg =
        secondHalf.reduce((sum, m) => sum + m.operationTime, 0) / secondHalf.length

      console.log(
        `Performance comparison: First half ${firstHalfAvg}ms vs Second half ${secondHalfAvg}ms`
      )
