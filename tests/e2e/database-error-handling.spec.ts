...
test.describe('Database Error Handling E2E Tests', () => {
  test.describe('Connection Error Recovery', () => {
    test('should handle database connection failures gracefully', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="db-error-banner"]')).toContainText(/database.*unavailable/i)
    ...
    test('should use cached data during database outage', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="cached-data-message"]')).toContainText(/cached data/i)
    })
  ...
  test.describe('Query Error Handling', () => {
    ...
    test('should handle constraint violations gracefully', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="field-error-name"]')).toContainText(/already exists/i)
  ...
  test.describe('Connection Pool Management', () => {
    test('should warn about connection pool exhaustion', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="pool-warning"]')).toContainText(/connection pool.*95%/i)
    ...
    test('should handle timeout errors appropriately', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="optimization-suggestions"]')).toContainText(/pagination.*indexes/i)
  ...
  test.describe('Migration Error Handling', () => {
    test('should handle migration failures safely', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="rollback-status"]')).toContainText(/rollback.*successful/i)
    ...
    test('should validate database schema before operations', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="schema-issue-0"]')).toContainText(/users.*email_verified/i)
      await expect(page.locator('[data-testid="schema-issue-1"]')).toContainText(/repositories.*idx_repo_created_at/i)
    })
  })
  ...
  test.describe('Backup and Recovery', () => {
    test('should handle backup failures', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="backup-error"]')).toContainText(/insufficient disk space/i)
    ...
    test('should validate restore operations', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="warning-0"]')).toContainText(/different database version/i)
