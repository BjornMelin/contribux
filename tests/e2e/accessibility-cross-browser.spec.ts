...
test.describe('Accessibility and Cross-Browser Compatibility', () => {
  ...
  test.describe('Accessibility Compliance', () => {
    ...
    test('should have proper ARIA labels and semantic markup', async ({ page }) => {
      ...
      const criticalIssues = ariaIssues.filter(issue => 
        issue.includes('Button without accessible name') ||
        issue.includes('Input without label')
      )
  ...
  test.describe('Cross-Browser Compatibility', () => {
    ...
    test('should handle network conditions gracefully', async ({ page }) => {
      ...
      // Try to navigate
      const response = await page.goto('/search').catch(() => null)
    ...
    test('should work on mobile browsers', async ({ page }) => {
      ...
      const mobileNav = page.locator('.mobile-nav, [data-testid="mobile-nav"], button[aria-label*="menu"]')
      ...
      for (const img of images) {
        const imgInfo = await img.evaluate(el => {
          ...
          return {
            ...
            displayWidth: parseInt(computedStyle.width),
            displayHeight: parseInt(computedStyle.height),
            src: el.src
          }
        })
