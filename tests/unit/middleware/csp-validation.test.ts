...
describe('CSP (Content Security Policy) Validation Tests', () => {
  ...
  describe('CSP Parser', () => {
    it('should parse CSP header correctly', () => {
      const cspHeader = "default-src 'self'; script-src 'self' 'unsafe-inline' https://cdn.example.com; img-src 'self' data: https:"
    ...
    it('should handle complex CSP directives', () => {
      const cspHeader = "default-src 'self'; script-src 'self' 'nonce-abc123' 'sha256-def456'; style-src 'self' 'unsafe-inline'; report-uri /csp-report"
