...
describe('securityHeadersMiddleware', () => {
  it('should apply headers to all responses', async () => {
    ...
    const result = await securityHeadersMiddleware(request, response)
  ...
  it('should use production headers in production', async () => {
  ...
  it('should use development headers in development', async () => {
    const originalEnv = process.env.NODE_ENV
...
describe('Header Configurations', () => {
  it('should have stricter production headers', () => {
    ...
    const prodCsp = productionSecurityHeaders.csp?.directives?.['script-src']
    expect(prodCsp).not.toContain("'unsafe-inline'")
    expect(prodCsp).not.toContain("'unsafe-eval'")

    // Production should enforce HTTPS
    expect(productionSecurityHeaders.hsts).toBeTruthy()
  })

  it('should have relaxed development headers', () => {
    // Development CSP should allow unsafe-inline and localhost
    const devCsp = developmentSecurityHeaders.csp?.directives?.['script-src']
    expect(devCsp).toContain("'unsafe-inline'")
    expect(devCsp).toContain("'unsafe-eval'")
    
    const devConnect = developmentSecurityHeaders.csp?.directives?.['connect-src']
    expect(devConnect).toContain('http://localhost:*')
  })
})
