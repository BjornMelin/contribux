...
import {
  getSecurityMockCallCounts,
  mockFailureScenarios,
  resetSecurityMocks,
} from './mocks/security-api-mocks'
...
describe('SOAR Integration', () => {
  ...
  describe('External Security System Integration', () => {
    ...
    it('should integrate with user management systems', async () => {
      ...
      const quarantineAction = await engine.executeResponseAction(
        'quarantine_user',
        'user-123',
        true
      )
    ...
    it('should integrate with evidence collection systems', async () => {
      ...
      const action = await engine.executeResponseAction(
        'collect_evidence',
        'compromised-host',
        true
      )
    ...
    it('should integrate with notification services', async () => {
      ...
      const notifyAction = await engine.executeResponseAction(
        'notify_stakeholders',
        'security-team',
        true
      )
      const escalateAction = await engine.executeResponseAction(
        'escalate_incident',
        'incident-123',
        true
      )
  ...
  describe('Performance and Scalability', () => {
    ...
    it('should maintain performance with multiple concurrent incidents', async () => {
      ...
      const incidents = Array(5)
        .fill(null)
        .map((_, index) => createMockSecurityIncident({ incidentId: `perf-test-${index}` }))
